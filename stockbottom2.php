...
if( isset($_GET['nid']) and is_numeric($_GET['nid']) ){
    global $user; if( isset($user->roles[3]) or isset($user->roles[4]) or isset($user->roles[5]) ){
        ...
        $body='';
            ...
            $body='<div style="margin-left: 317px;"><a target="_blank" href="/node/'.$_GET['nid'].'/edit">Редактировать товар</a>';
            ...
            if( isset($uri['body_value']) and strlen($uri['body_value']) ){
                $body.='<div style="border: 1px solid #929191; margin-top: 19px; padding: 11px; overflow: auto; height: 157px;">'.$uri['body_value'].'</div>';
            }
            $body.='</div>';
