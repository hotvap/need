        ...
        global $user; if($user->uid){
            ...
            if( isset($res['nid']) and is_numeric($res['nid']) ){
                ...
                if(!isset($hit['field_delete_value'])){
                    ...
                    $hit=db_query('select field_reason2_value from {field_data_field_reason2} where bundle=\'deal\' and entity_id='.$_POST['nid']);
                    $hit=$hit->fetchAssoc();
                    if(!isset($hit['field_reason2_value'])){
                        db_query('insert into {field_data_field_reason2} (entity_type, bundle, deleted, entity_id, revision_id, language, delta, field_reason2_value) values (\'node\',\'deal\',0,'.$_POST['nid'].','.$_POST['nid'].',\'und\',0,:combo)', array(':combo'=>$_POST['dt']));
                    }else{
                        db_query('update {field_data_field_reason2} set field_reason2_value=:combo where bundle=\'deal\' and entity_id='.$_POST['nid'], array(':combo'=>$_POST['dt']));
                    }

                    $hit=db_query('select field_rat2_value from {field_data_field_rat2} where bundle=\'deal\' and entity_id='.$_POST['nid']);
                    $hit=$hit->fetchAssoc();
                    if(!isset($hit['field_rat2_value'])){
                        db_query('insert into {field_data_field_rat2} (entity_type, bundle, deleted, entity_id, revision_id, language, delta, field_rat2_value) values (\'node\',\'deal\',0,'.$_POST['nid'].','.$_POST['nid'].',\'und\',0,:combo)', array(':combo'=>$_POST['rat']));
                    }else{
                        db_query('update {field_data_field_rat2} set field_rat2_value=:combo where bundle=\'deal\' and entity_id='.$_POST['nid'], array(':combo'=>$_POST['rat']));
                    }

                    $hit=db_query('select field_end1_value from {field_data_field_end1} where bundle=\'deal\' and entity_id='.$_POST['nid']);
                    $hit=$hit->fetchAssoc();
                    if(!isset($hit['field_end1_value'])){
                        db_query('insert into {field_data_field_end1} (entity_type, bundle, deleted, entity_id, revision_id, language, delta, field_end1_value) values (\'node\',\'deal\',0,'.$_POST['nid'].','.$_POST['nid'].',\'und\',0,:combo)', array(':combo'=>1));
                    }else{
                        db_query('update {field_data_field_end1} set field_end1_value=:combo where bundle=\'deal\' and entity_id='.$_POST['nid'], array(':combo'=>1));
                    }
                    ...
                    if( isset( $ismail['mail'] ) and strlen($ismail['mail']) and valid_email_address($ismail['mail']) and function_exists('pdxmail') ){
                        ...
                        $msg='<p>Арендодатель объявил о завершении сделки по товару "<a href="'.url('node/'.$ismail['nid'], array('absolute'=>true)).'">'.$ismail['title'].'</a>". О Вас был оставлен следующий отзыв:</p>';
                        ...
                        if( function_exists('pdxupdateuserrat') ){
                            pdxupdateuserrat($ismail['uid'], 'field_rat2');
                    ...
                    $hit=db_query('select field_end2_value from {field_data_field_end2} where bundle=\'deal\' and field_end2_value=1 and entity_id='.$_POST['nid']);
                    $hit=$hit->fetchAssoc();
                    if(!isset($hit['field_end2_value'])){
                        echo 'Спасибо, Вы завершили сделку. Ждем завершения сделки от арендатора.';
                    }else{
