...
//    $out.= '<pre>'.print_r($product, true). '</pre>';
    $out.= $product->order_id.'^'; //OrderID
    $out.= $product->order_total.'^'; //TotalSum
    $out.= '0^'; //Discount
    $out.= $product->created.'^'; //OrderDate

    $street='';
    if( isset($product->delivery_city) and strlen($product->delivery_city) ){
        $street.= $product->delivery_city;
    }
    if( isset($product->delivery_street1) and strlen($product->delivery_street1) ){
        if( strlen($street) ){
            $street.= ', ';
        }
        $street.= $product->delivery_street1;
    }
    if( isset($product->delivery_street2) and strlen($product->delivery_street2) ){
        if( strlen($street) ){
            $street.= ', ';
        }
        $street.= $product->delivery_street2;
    }
    $out.= string_moisklad($street).'^'; //Adress
    $out.= $product->delivery_phone.'^'; //Phone

    $street='';
    if( isset($product->delivery_first_name) and strlen($product->delivery_first_name) ){
        $street.= $product->delivery_first_name;
    }
    if( isset($product->delivery_last_name) and strlen($product->delivery_last_name) ){
        if( strlen($street) ){
            $street.= ' ';
        }
        $street.= $product->delivery_last_name;
    }
    $out.= string_moisklad($street).'^'; //CustomerName
    $out.= '0
'; //CardNo
...
header("Content-Disposition: attachment; filename=txtorderheader.txt");    
