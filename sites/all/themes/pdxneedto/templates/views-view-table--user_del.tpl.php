  ...
  <?php if (!empty($header)) : ?>
    <thead>
      <tr>
        ...
        if($field=='field_moderate_reason'){ continue; }
         ?>
          ...
          <td class="<?php echo $field.' '; if ($field_classes[$field][$row_count]) { print $field_classes[$field][$row_count]; } ?>" <?php print drupal_attributes($field_attributes[$field][$row_count]); ?>>
            ...
            case 'field_block':
                if( isset($content) and $content==1 ){
                    echo '<img alt="" src="/sites/all/themes/pdxneedto/img/yes.png" />';
                    if( isset( $row['field_moderate_reason'] ) and strlen( $row['field_moderate_reason'] ) ){
                        echo ' <strong>Причина: </strong>'.$row['field_moderate_reason'];
                    }
                }else{
                    echo '&nbsp;';
                }
