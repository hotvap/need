  ...
  <tbody>
    <?php foreach ($rows as $row_count => $row): ?>
      ...
      if( isset( $row['title'] ) and strlen( $row['title'] ) ){
        echo 'title="'.str_replace('"','',strip_tags($row['title'])).'" ';
      }
      if ($row_classes[$row_count]) { print 'class="' . implode(' ', $row_classes[$row_count]) .'"';  } ?>>
        <?php foreach ($row as $field => $content): ?>
          <td class="<?php echo $field.' '; if ($field_classes[$field][$row_count]) { print $field_classes[$field][$row_count]; } ?>" <?php print drupal_attributes($field_attributes[$field][$row_count]); ?>>
            ...
            case 'uid':
                ...
                if( isset($content) and is_numeric($content) and $content>0 ){
                    $name=db_query('select field_name_value from {field_data_field_name} where entity_type=\'user\' and entity_id='.$content);
                    $name=$name->fetchAssoc();
                    if( isset($name['field_name_value']) and strlen($name['field_name_value']) ){
                        echo '<a href="/user/'.$content.'">'.$name['field_name_value'].'</a>';
                    }else{
                        echo '<a href="/user/'.$content.'">'.$content.'</a>';
                    }
                }else{
                    echo $content;
                }
