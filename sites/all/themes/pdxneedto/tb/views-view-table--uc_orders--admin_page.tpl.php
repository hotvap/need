...
*/
            ...
            case 'product_count':
                ...
                if( isset($row['order_id']) and is_numeric($row['order_id']) ){
                    $nids=db_query('select title, model, nid, price from {uc_order_products} where order_id='.$row['order_id']);
                    while( $nid=$nids->fetchAssoc() ){
                        echo '<li><a href="/node/'.$nid['nid'].'"><strong>'.$nid['model'].'</strong> '.$nid['title'].'</a><span>'.uc_currency_format($nid['price']).'</span></li>';
                    }
                }
                echo '</ul>';
                break;
