  <div class="region_item" id="region_region"><div class="region_admin_label" onclick=" jQuery('.admin_region_content').toggle(); "> -- SEO настройки страницы -- </div><div class="admin_region_content" style="display: none;">
  <?php
...
if($modified){
    ...
    $fp=fopen('pdxcache/modified/'.implode('_',arg()), "w"); if($fp){ fwrite($fp, ''); fclose($fp); }
}
...
if( isset($_POST['setsynonym']) and strlen($_POST['setsynonym']) ){
    ...
    if(mb_substr($_POST['setsynonym'], mb_strlen($_POST['setsynonym'])-1 )=='/'){
        $_POST['setsynonym']=mb_substr($_POST['setsynonym'], 0, mb_strlen($_POST['setsynonym'])-1 );
    }
    if( mb_strpos($_POST['setsynonym'],'http://kit.by/www.')===false ){}else{
        $_POST['setsynonym']=str_replace('http://kit.by/www.','',$_POST['setsynonym']);
    }
    if( mb_strpos($_POST['setsynonym'],'http://kit.by/')===false ){}else{
        $_POST['setsynonym']=str_replace('http://kit.by/','',$_POST['setsynonym']);
    }
    if( mb_strpos($_POST['setsynonym'],'http://')===false ){}else{
        $_POST['setsynonym']=str_replace('http://','',$_POST['setsynonym']);
    }
    if( mb_strpos($_POST['setsynonym'],'kit.by/')===false ){}else{
        $_POST['setsynonym']=str_replace('kit.by/','',$_POST['setsynonym']);
    }
